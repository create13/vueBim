function GeometryLoader() {
	...
	this.createGeometry = function(geometryId,vertices, normals, colors, indices,transformationMatrix,ifcProductOid,material){
		...
		var object = new THREE.Mesh(geometry,new THREE.MeshLambertMaterial( { color: new THREE.Color(material.r,material.g,material.b),opacity:material.a,transparent:true,side: THREE.DoubleSide} ));
	...
	this.createGeometryLine = function(geometryId,vertices,indices){
		...
		geometry.addAttribute( 'position', new THREE.BufferAttribute( vertices, 3 ) );
		geometry.setIndex( new THREE.BufferAttribute( indices, 1 ) );
		o.gId["Line." + geometryId] = new THREE.LineSegments(geometry,new THREE.LineBasicMaterial({ color: 0x404040, linewidth: 1 ,opacity:.3,transparent:true}));
	};
	
	this.createBimObject = function(ifcProductOid , geometryDataOid , material, transformationMatrix){
	...
	this.readObject = function(data, geometryType) {
		...
		if (geometryType == 1) {
			...
			o.createBimObject( ifcProductOid ,geometryDataOid, material, transformationMatrix);
			o.createBimLine(ifcProductOid ,geometryDataOid, material, transformationMatrix);
			...
			o.createBimObject(ifcProductOid ,geometryIds.toString(), material, transformationMatrix);
		}else if(geometryType == 3){
            var geometryDataOid = data.readLong();
            o.createBimObject(ifcProductOid ,geometryDataOid, material, transformationMatrix);
            o.createBimLine( ifcProductOid ,geometryDataOid, material, transformationMatrix);
		}else{
            for (var i=0,arraySize = data.readInt(),coreIds = [];i<arraySize;i++) {
                coreIds.push(data.readLong());
            }
            o.createBimObject(ifcProductOid ,coreIds.toString(), material, transformationMatrix);
		}
